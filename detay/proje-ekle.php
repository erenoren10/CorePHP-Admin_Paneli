...
<option value="0" <?php if($guncelle["kategori"]==0){ echo "selected";}?>>Kategori Yok</option>
<?php
$katcek = $db->query("select * from proje_kategori order by id desc")->fetchAll(PDO::FETCH_ASSOC);
foreach($katcek as $kat){?>
<option value="<?=$kat["id"]?>" <?php if($guncelle["kategori"]==$kat["id"]){ echo"selected";}?>><?=$kat["adi"]?></option>
<?php }?>
</select>
</div>
</div>
...
<div class="col-lg-12">
<div class="form-group">
<label> Proje Öne Çıkan Resmi </label>
...
<textarea  class="ckeditor" name="aciklama"  rows="10"><?=$guncelle['aciklama']?></textarea>
                            	...
                            	<?php
									...
									$iddd=$_GET['id'];
									if($_GET['islem']=='duzenle'){
										$cek = $db->query("SELECT * FROM urun_img WHERE urun_id = '$iddd' and tur='projeler' order by id asc", PDO::FETCH_ASSOC);
										if($cek->rowCount()){
											foreach( $cek as $c ){
                             ...
                             <div class="col-md-4 " style="margin:auto;padding:auto;">
				                    <div class="uploaddis aktif" data-id="1" style="margin:0 auto;">
				        			  <div class="upload1">
				        				  <span class="metin" style="width: 100%;float: left;">Proje Resimler Yükle</span>
...
<div class="col-lg-12">
 <?php if($_GET['islem']=='duzenle'){?>
<button class="btn btn-submit me-2" type="submit" name="proje-guncelle">Proje Güncelle</button>
...
<button class="btn btn-submit me-2" type="submit" name="proje-ekle">Proje Ekle</button>
